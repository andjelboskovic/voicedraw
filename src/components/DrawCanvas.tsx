      ...
      >
        ...
        {sortedObjects.map((obj: SceneObject) => {
          ...
          return (
            <g
              key={obj.id}
              transform={`translate(${t.x - 150 * t.scale},${t.y - 100 * t.scale}) scale(${t.scale}) rotate(${t.rotate})`}
              style={{
        ...
        {isNewCreate && streamingIntent && (
          <g
            transform={`translate(${(streamingIntent.transform?.x ?? 500) - 150 * (streamingIntent.transform?.scale ?? 1)},${(streamingIntent.transform?.y ?? 350) - 100 * (streamingIntent.transform?.scale ?? 1)}) scale(${streamingIntent.transform?.scale ?? 1}) rotate(${streamingIntent.transform?.rotate ?? 0})`}
